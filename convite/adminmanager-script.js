    ...
    function applyRoleUi(){ document.body.classList.toggle('role-editor', isEditor()); document.body.classList.toggle('role-admin', isAdmin()); $('connectionLabel').textContent = isEditor() ? 'Perfil Editor · edição limitada' : 'Perfil Admin · acesso total'; }
    ...
    function fillGuestForm(g={}){
      ...
      $('guestResetAccessSingleBtn').style.display = (editingGuestId && isAdmin()) ? '' : 'none';
    ...
    function openNewGuest(){ if(!isAdmin()) return showMsg($('guestCrudResult'), 'Perfil Editor não pode criar convidados.', true); const inv=selectedGuestInvite(); if(!inv) return showMsg($('guestCrudResult'), 'Seleccione primeiro um convite.', true); fillGuestForm({ table:'A indicar na entrada', status:'Não aberto' }); openGuestModal(); }
    async function saveGuestFromModal(e){
      ...
      if(!editingGuestId && !isAdmin()) return showMsg($('guestEditResult'), 'Perfil Editor não pode criar convidados.', true);
      try{
    ...
    function bindEvents(){
      ...
      $('repairGuestsBtn').addEventListener('click', async ()=>{
        if(!isAdmin()) return showMsg($('guestResult'), 'Perfil Editor não pode executar reparação global.', true);
      ...
      $('guestForm').addEventListener('submit', async e=>{ if(!isAdmin()) { e.preventDefault(); return showMsg($('guestResult'), 'Perfil Editor não pode importar/criar convidados.', true); }
        e.preventDefault();
        const id = $('guestInviteSelect').value;
