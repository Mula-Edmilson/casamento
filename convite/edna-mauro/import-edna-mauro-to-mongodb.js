...
const SLUG = process.env.INVITE_SLUG || 'edna-mauro';
...
async function main() {
  ...
  const invite = await Invite.findOneAndUpdate(
    { slug: SLUG },
    {
      $set: {
        slug: SLUG,
        clientName: event.clientName || event.groom || 'Edna Mondlane',
        coupleNames: event.coupleNames || 'Edna & Mauro',
        bride: event.bride || 'Edna Otília Mondlane',
        groom: event.groom || 'Mauro Damison Macamo',
  ...
  const isOpenInvite = String(event.guestAccessMode || '').toLowerCase() === 'open';
  ...
  // Em convites abertos, os convidados podem ser criados pelas interacções públicas.
  // Nunca os apagamos numa reimportação do seed.
  if (!isOpenInvite) {
