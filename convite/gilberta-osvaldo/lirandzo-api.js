...
// Convite: Osvaldo & Gilberta | slug: gilberta-osvaldo
(function () {
  'use strict';
...
// Compatibilidade com o painel de cliente Lirandzo: expõe LIRANDZO_API sobre o cliente Render/MongoDB.
(function () {
  ...
  async function adminPost(action, payload) {
    const body = Object.assign({}, payload || {}, {
      action,
      slug: slug(),
      password: sessionStorage.getItem('lirandzo_admin_password') || '',
      admin_password: sessionStorage.getItem('lirandzo_admin_password') || ''
    });
    const headers = { 'Content-Type': 'application/json', 'X-Invite-Slug': slug() };
    const token = sessionStorage.getItem('lirandzo_admin_token') || '';
  ...
  window.LIRANDZO_API = {
    ...
    async authenticateAdmin(password) {
      ...
      //    ou variáveis por slug como INVITE_ADMIN_PASSWORD_GILBERTA_OSVALDO).
      const cleanPassword = String(password || '').trim();
      let token = '';
      sessionStorage.setItem('lirandzo_admin_password', cleanPassword);
      sessionStorage.removeItem('lirandzo_admin_token');
      try {
        token = await managerLogin(cleanPassword);
        this.ADMIN_TOKEN = token;
        if (token) sessionStorage.setItem('lirandzo_admin_token', token);
        return { status: 'success', token };
      } catch (loginErr) {
        // Fallback do painel: valida a senha directamente no admin-api do convite.
