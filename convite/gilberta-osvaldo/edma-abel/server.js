...
function applyTemplateReplacements(content, ctx, filePath = '') {
  ...
  const replacements = {
    gilberta-osvaldo: ctx.slug, https://api-casamento-mj.onrender.com: ctx.apiBaseUrl, https://api-casamento-mj.onrender.com/api: `${ctx.apiBaseUrl}/api`,
    Gilberta & Osvaldo: ctx.coupleNames, Gilberta Bucuane: ctx.clientName, Gilberta: ctx.bride || '',
    Osvaldo: ctx.groom || '', esmeralda: ctx.packageKey, https://lirandzo.com/convite/gilberta-osvaldo/: ctx.publicUrl
  };
  ...
  out = out.replace(/const\s+API_URL\s*=\s*[`'\"][^`'\"]*[`'\"]\s*;/g, `const API_URL = "https://api-casamento-mj.onrender.com/api";`);
  out = out.replace(/const\s+DEMO_MODE\s*=\s*true\s*;/g, 'const DEMO_MODE = false;');
  out = out.replace(/BASE_URL\s*:\s*[`'\"][^`'\"]*[`'\"]/g, `BASE_URL: "https://api-casamento-mj.onrender.com/api"`);
