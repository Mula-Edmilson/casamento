    ...
    (function () {
      ...
      async function adminPost(action, payload = {}) {
        ...
        if (!res.ok || data.status === 'error') {
          if (res.status === 401) {
            ...
            showToast('Sessão expirada. Por favor entre novamente.', 'warn');
        ...
        loginForm.addEventListener('submit', async (e) => {
          ...
          try {
            ...
            if (apiBase()) {
              ...
              if (!window.LIRANDZO_ADMIN_PASSWORD) throw new Error('API não configurada e senha local não definida. Configure LIRANDZO_API_BASE_URL ou LIRANDZO_ADMIN_PASSWORD.');
              if (password !== window.LIRANDZO_ADMIN_PASSWORD) throw new Error('Senha incorrecta. Verifique a senha e tente novamente.');
            }
      ...
      async function handleAddGuestSubmit(event) {
        ...
        try {
          ...
          showToast('Convidado adicionado com sucesso.', 'ok');
          await loadAll();
          showPanel('guests');
        } catch (err) {
          showToast(err.message || 'Não foi possível adicionar o convidado.', 'error');
        } finally {
      ...
      async function handleAdminActionClick(event) {
        ...
        if (!id) { showToast('ID do registo não encontrado.', 'error'); return; }
        ...
        if (action === 'hide_message') {
          ...
          confirmText = `Eliminar definitivamente a mensagem de "${name}"? Esta acção não pode ser desfeita.`;
        ...
        const confirmed = await showConfirm(confirmText, confirmWord);
        if (!confirmed) return;
        ...
        try {
          ...
          showToast(err.message || 'A operação falhou.', 'error');
        } finally {
      ...
      function setDataSourceState() {
        ...
        backendNote.innerHTML = configured
          ? 'Painel sincronizado. Os dados são actualizados sempre que houver novas confirmações, presentes escolhidos ou mensagens.'
      ...
      async function loadAll() {
        ...
        try {
          ...
          // Presentes: usar apenas rotas antigas e estáveis.
          // Não carregar PDFs, imagens, thumbnails ou fileBase64.
          const giftsPromise = trySources([
            () => adminPost('get_gifts'),
            () => adminPost('get_comprovativos'),
            () => LirandzoAPI.get('list_gifts'),
            () => LirandzoAPI.get('list_gift_records')
          ], { status: 'success', data: [] });
      ...
      function renderMetrics() {
        const s = summary();
        const metrics = [
          ['Convidados', s.guestsCount, `${s.totalPeople} pessoas no total`],
          ...
          ['Mensagens', s.messages, 'felicitações recebidas'],
      ...
      function renderCharts() {
        ...
        // RSVP e check-in usam a mesma unidade: registos (convidados), não pessoas
        makeChart('rsvpChart', ['Confirmados', 'Pendentes'], [s.confirmedRows, s.pendingRows], [cssVar('--accent') || '#b08968', empty]);
        makeChart('checkinChart', ['Entraram', 'Por entrar'], [s.checkedRows, Math.max(s.guestsCount - s.checkedRows, 0)], [cssVar('--green') || '#18b783', empty]);
      ...
      function renderActivity() {
        const items = [
          ...
          ...DATA.gifts.map(x => ({ ...x, type: 'Presente', icon: 'gift', detail: giftDetail(x) })),
      ...
      function exportCSV(type) {
        ...
        const rows = map[type] || [];
        if (!rows.length) { showToast('Sem dados para exportar.', 'warn'); return; }
        const keys = [...new Set(rows.flatMap(row => Object.keys(row || {})))];
        const csv = [keys.join(','), ...rows.map(row => keys.map(key => '"' + String(row[key] ?? '').replace(/"/g, '""') + '"').join(','))].join('\n');
        // BOM UTF-8 \uFEFF garante que o Excel abre correctamente caracteres especiais (ã, ç, é)
      ...
      // --- Toast & Modal (substitui alert/confirm/prompt nativos) ---
      ...
      function showToast(message, type = 'ok') {
        ...
        if (!container) {
          ...
          container.style.cssText = 'position:fixed;bottom:24px;right:20px;z-index:9999;display:grid;gap:8px;pointer-events:none;';
          document.body.appendChild(container);
        }
        const colors = { ok: 'var(--green)', error: 'var(--red)', warn: 'var(--orange)', info: 'var(--blue)' };
        const toast = document.createElement('div');
        toast.style.cssText = `min-width:240px;max-width:340px;background:var(--surface);border:1px solid ${colors[type] || colors.info};border-radius:14px;padding:11px 14px;color:var(--text);font-size:13px;font-weight:700;box-shadow:0 8px 24px rgba(0,0,0,.28);pointer-events:auto;opacity:0;transform:translateY(8px);transition:opacity .22s ease,transform .22s ease;`;
      ...
      function showConfirm(message, confirmWord = '') {
        return new Promise(resolve => {
          let modal = $('confirmModal');
          if (!modal) {
            modal = document.createElement('div');
            modal.id = 'confirmModal';
            modal.style.cssText = 'position:fixed;inset:0;z-index:8000;display:flex;align-items:center;justify-content:center;padding:20px;background:rgba(0,0,0,.6);backdrop-filter:blur(6px);';
            modal.innerHTML = `
              <div style="width:min(420px,96vw);background:var(--surface);border:1px solid var(--line-strong);border-radius:22px;padding:22px;box-shadow:var(--shadow);">
                <p id="confirmModalMsg" style="color:var(--text);font-size:14px;line-height:1.55;margin-bottom:16px;"></p>
                <div id="confirmModalWordWrap" style="display:none;margin-bottom:14px;">
                  <label style="display:block;color:var(--accent-2);font-size:10px;font-weight:900;letter-spacing:.08em;text-transform:uppercase;margin-bottom:6px;">Escreva <strong id="confirmModalWordLabel"></strong> para confirmar</label>
                  <input id="confirmModalInput" type="text" style="width:100%;min-height:40px;border:1px solid var(--line-strong);border-radius:12px;background:rgba(33,28,25,.9);color:var(--text);padding:0 12px;font-size:13px;outline:none;">
                </div>
                <div style="display:flex;gap:8px;justify-content:flex-end;">
                  <button id="confirmModalCancel" class="btn secondary small" type="button">Cancelar</button>
                  <button id="confirmModalOk" class="btn small" type="button" style="background:linear-gradient(135deg,var(--red),#c73e3e);">Confirmar</button>
                </div>
              </div>`;
            document.body.appendChild(modal);
          }
          const msg = $('confirmModalMsg');
          const wordWrap = $('confirmModalWordWrap');
          const wordLabel = $('confirmModalWordLabel');
          const wordInput = $('confirmModalInput');
          const cancelBtn = $('confirmModalCancel');
          const okBtn = $('confirmModalOk');
          msg.textContent = message;
          if (confirmWord) {
            wordWrap.style.display = 'block';
            wordLabel.textContent = confirmWord;
            wordInput.value = '';
          } else {
            wordWrap.style.display = 'none';
          }
          modal.style.display = 'flex';
          if (confirmWord) wordInput.focus();
          ...
          $('confirmModalCancel').addEventListener('click', () => cleanup(false), { once: true });
          $('confirmModalOk').addEventListener('click', () => {
            if (confirmWord && $('confirmModalInput').value.trim() !== confirmWord) {
              $('confirmModalInput').style.borderColor = 'var(--red)';
              showToast(`Escreva exactamente: ${confirmWord}`, 'error');
              return;
            }
            cleanup(true);
          }, { once: true });
          modal.addEventListener('keydown', (e) => { if (e.key === 'Escape') cleanup(false); }, { once: true });
        });
      }
