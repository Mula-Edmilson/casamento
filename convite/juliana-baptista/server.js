...
function isExplicitlyNotOpenedStatus(value) {
  ...
  return status === 'nao aberto' || status.includes('nao aberto') || status === 'pendente' || status === 'not opened' || status === 'unopened';
}
function isGuestOpenedStatus(value) {
  const status = normalizeText(value);
  if (!status || isExplicitlyNotOpenedStatus(status)) return false;
  return status === 'aberto' || status.includes('convite aberto') || status.includes('confirm') || status.includes('check') || status.includes('entrou') || status.includes('opened') || status.includes('visualiz') || status.includes('access');
}
...
const INVITE_SPECIFIC_GIFTS = {
  'juliana-baptista': ['Blocos', 'Cimento', 'Ferro (Varão)'],
  'amelia-edilson': [
    'Microondas',
    'Taças de cristal',
    'Jogo de banho Maria',
...
function giftSeedListForInvite(invite) {
  ...
  const configOptions = Array.isArray(invite?.config?.giftOptions) ? invite.config.giftOptions.map(item => String(item?.name || item || '').trim()).filter(Boolean) : [];
  const custom = configOptions.length ? configOptions : (INVITE_SPECIFIC_GIFTS[slug] || []);
  return Array.from(new Set(custom.length ? custom : DEFAULT_GIFTS));
}
...
app.post('/api', upload.any(), async (req, res) => {
  try {
    ...
    if (action === 'save_gift_contributions' || action === 'save_quantity_gifts') return handleSaveGiftContributions(req, res, invite);
...
async function getPublicStats(req, res, invite) {
  ...
  const openedRows = guests.filter(g => isGuestOpenedStatus(g.status) || Boolean(g.checkedIn)).length;
...
function isGiftContributionChannel(row) {
  const channel = normalizeGiftKeyServer(row?.canal || '');
  return channel.includes('presente escolhido') || channel.includes('contribuicao de presente');
}
...
async function getGuestDetails(req, res, invite) {
  ...
  if (!alreadyConfirmedOrChecked && !isGuestOpenedStatus(currentStatus)) {
    guest.status = 'Convite Aberto';
    await guest.save();
...
function quantityGiftOptionsForInvite(invite) {
  const rows = Array.isArray(invite?.config?.giftOptions) ? invite.config.giftOptions : [];
  return rows.map(item => ({
    name: String(item?.name || item || '').trim(),
    step: Math.max(1, Number(item?.quantityStep || item?.step || 1)),
    min: Math.max(1, Number(item?.minQuantity || item?.quantityStep || 1)),
    unit: String(item?.unit || 'unidade').trim()
  })).filter(item => item.name);
}

async function handleSaveGiftContributions(req, res, invite) {
  if (String(invite?.config?.giftSelectionMode || '') !== 'quantity_contributions') {
    return res.status(400).json({ status: 'error', message: 'Este convite não utiliza contribuições por quantidade.' });
  }

  const body = req.body || {};
  const rawName = String(body.nome || body.guestName || body.convidado || '').trim();
  const rawToken = String(body.token || body.guestToken || body.inviteToken || '').trim();
  if (!rawName && !rawToken) return res.status(400).json({ status: 'error', message: 'Nome ou token do convidado é obrigatório.' });

  const guest = await findGuestByIdentity(invite, { nome: rawName, token: rawToken });
  if (!guest) return res.status(404).json({ status: 'error', message: 'Convidado não encontrado.' });

  const optionMap = new Map(quantityGiftOptionsForInvite(invite).map(item => [compactGiftKeyServer(item.name), item]));
  const requested = Array.isArray(body.giftContributions) ? body.giftContributions : [];
  if (!requested.length) return res.status(400).json({ status: 'error', message: 'Seleccione pelo menos uma quantidade.' });

  const contributions = [];
  for (const raw of requested) {
    const key = compactGiftKeyServer(raw?.giftName || raw?.name || raw?.gift || '');
    const option = optionMap.get(key);
    if (!option) return res.status(400).json({ status: 'error', message: 'Foi enviada uma opção de contribuição inválida.' });
    const quantity = Number(raw?.quantity);
    if (!Number.isInteger(quantity) || quantity < option.min || quantity % option.step !== 0) {
      return res.status(400).json({ status: 'error', message: `${option.name}: use múltiplos de ${option.step}.` });
    }
    contributions.push({ ...option, quantity });
  }

  const now = new Date();
  const saved = [];
  for (const item of contributions) {
    const details = { selectedGift: item.name, giftChoice: item.name, selectedGifts: [item.name], quantity: item.quantity, giftQuantity: item.quantity, unit: item.unit, quantityStep: item.step, source: 'quantity_contributions' };
    const identityFilter = rawToken ? { token: rawToken } : { nome: guest.name };
    const row = await Contribution.findOneAndUpdate(
      { inviteId: invite._id, canal: 'Contribuição de presente', selectedGift: item.name, ...identityFilter },
      { $setOnInsert: { inviteId: invite._id, slug: invite.slug, canal: 'Contribuição de presente', timestamp: now }, $set: { nome: guest.name, token: rawToken || guest.inviteToken || '', selectedGift: item.name, giftChoice: item.name, selectedGifts: [item.name], gifts: item.name, details, timestamp: now } },
      { upsert: true, new: true, runValidators: true }
    );
    saved.push({ id: String(row._id), giftName: item.name, quantity: item.quantity, unit: item.unit });
  }

  await logActivity({ invite, type: 'gift', title: 'Contribuição de presente', detail: `${guest.name} · ${saved.map(item => `${item.giftName}: ${item.quantity}`).join(', ')}`, meta: { guest: guest.name, contributions: saved } });
  return res.json({ status: 'success', message: 'Contribuição registada com sucesso.', data: saved });
}
